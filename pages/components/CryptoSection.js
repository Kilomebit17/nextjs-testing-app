...
const CryptoSection = ({data}) => {
    const cryptoItemNodeList = data.crypto.crypto.map(({id, name, price, description, percent}) => {
        return <CryptoItem key={id} name={name} price={price} description={description} percent={percent}/>;
    });
    return (
        <div className={style.main}>
            <div className={style.main__andyInfo}>
                <div className={style.andyLogo}>
                    <Image src='/media/Andy.png' alt='Picture of the author' width={115} height={115}/>
